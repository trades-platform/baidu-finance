"""Sector data: industries (SW2), concepts, and HK sectors.
...
from .models import CONNECT_COLUMNS, CONSTITUENT_COLUMNS, SECTOR_COLUMNS
...
_HK_BLOCKS_URL = (
    "https://finance.pae.baidu.com/vapi/v2/blocks"
    "?style=heatmap&market=hk&typeCode=HSHY&sortKey=amount&sortType=desc"
    "&pn=0&rn=100&finClientType=pc&finClientType=pc"
)
_BLOCK_KLINE_URL = (
...
class SectorAPI:
    """Industry, concept, and HK-sector listings, constituents, and K-line."""
    ...
    def hk_sector_constituents(self, name: str) -> pd.DataFrame:
        ...
        return self._hk_constituents(info["real_code"], info["market"])
    ...
    def _hk_constituents(self, code: str, market: str) -> pd.DataFrame:
        res = self._transport.get_json(
            _HK_CONSTITUENT_URL.format(code=code, market=market, pn=0, rn=500)
        )
        body = res.get("Result", {}).get("list", {}).get("body", [])
        rows = [[x.get("code", ""), x.get("name", "")] for x in body if x.get("code")]
        return pd.DataFrame(rows, columns=CONSTITUENT_COLUMNS)
